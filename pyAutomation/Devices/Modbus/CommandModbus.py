...
class CommandModbus:
    ...
    def processReply(data_bytes: 'bytearray') -> 'str':
        ...
        if (b.len() < 10):
            status = (f"Server reply is too short to process: "
              f"{Point.bytesToHex(b)}")
            commandFailed()
            return status
        ...
        if data_bytes[7] == -0x7f:
            ...
            if data_bytes[8] == 0x01:
                status = (
                  "Illegal Function: The function code received in "
                  "the query is not allowed or invalid."
                )

            elif data_bytes[8] == 0x02:
                status = (
                  "Illedgal Data Address: The data address received "
                ...
                status = (
                  "Illegal Data Value: A value contained in the query "
                ...
                status = (
                  "Slave/Server Device Failure: The server failed "
                ...
                status = (
                  "Acknowledge: The slave/server has accepted the "
                ...
                status = (
                  "Slave/Server Device Busy: The slave is engaged in "
                ...
                status = (
                  "Negative Acknowledge: The slave cannot perform the "
                ...
                status = (
                  "Memory Parity Error: The slave attempted to read "
                ...
                status = (
                  "Gateway Problem: The target device failed to "
        ...
        if self.command == "COIL" and self.write is True:
            ...
            if (not data_bytes.equals(command_chars)) {
                status = ("Command 05 did not execute properly. Should have "
                  f"been: {Point.bytesToHex(command_chars)} but it was: "
                  f"{Point.bytesToHex(data_bytes)}")
            return

        if self.command == "DISCRETE" and self.write is True:
            assert False, "Cannot write to DISCRETE points"

        if self.command == "HOLDING" and self.write is True:
        ...
        if self.command == "INPUT" and self.write is True:
          ...
          and self.write is False:
            ...
            if type(point) == PointDiscrete:
                ...
                assert match is not None, (
                  f"Point {point.name} has an invalid "
                  f"address for a bool-in-word"
                )
    ...
    def buildCommandBytes(points: 'List[Point]') {
        ...
        if self.write is False:
            ...
            if self.command == 'COIL':  # COILS
                ...
                assert False f"Invalid command of {self.command} given"

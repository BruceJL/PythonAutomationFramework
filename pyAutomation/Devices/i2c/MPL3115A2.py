...
class MPL3115A2(i2cPrototype, PointHandler):
    ...
    def _setup(self):
        try:
            # ------------
            # bit 0 = SBYB
            # This bit is sets the mode to ACTIVE, where the system will make measurements
            # at periodic times based on the value of ST bits.
            ...
            # active, setting the OST bit will initiate an immediate measurement, the
            # part will then return to acquiring data as per the setting of the ST bits
            # in CTRL_REG2. In this mode, the OST bit does not clear itself and must be
            ...
            # OST bit and comes back to STANDBY mode. User shall read the value of the
            # OST bit before writing to this bit again
            bit_1 = 0

            # -----------
            # bit 2 = RST
            # Software Reset. This bit is used to activate the software reset. The Boot
            # mechanism can be enabled in STANDBY and ACTIVE mode. When the Boot bit is
            # enabled the boot mechanism resets all functional block registers and loads
            # the respective internal registers with default values. If the system was
            # already in STANDBY mode, the reboot process will immediately begin; else if
            # the system was in ACTIVE mode, the boot mechanism will automatically
            # transition the system from ACTIVE mode to STANDBY mode, only then can the
            # reboot process begin.

            # The I2C communication system is reset to avoid accidental corrupted data access.
            # At the end of the boot process the RST bit is de-asserted to 0. Reading this bit
            ...
            # -----------
            # bits 3-5 OS
            # Oversample Ratio. These bits select the oversampling ratio. Value is 2^OS.
            # The default value is 000 for a ratio of 1.
            # 111 = 512ms sample time.
            bit_345 = 7

            # -----------
            # bit 6 RAW
            # RAW output mode. RAW bit will output ADC data with no post processing,
            # except for oversampling. No scaling or offsets will be applied in the
            # digital domain. The FIFO must be disabled and all other functionality:
            # Alarms, Deltas, and other interrupts are disabled.
            bit_6 = 0
            ...
            byte = bit_7 << 7 | bit_6 << 6 | bit_345 << 3 | bit_2 << 2 | bit_1 << 1 | bit_0
            ...
            # -----------
            # bit 2 DREM
            # Data ready event mode. If the DREM bit is set logic 1 and one or more of
            # the data ready event flags (PDEFE, TDEFE) are enabled, then an event flag
            # will be raised upon change in state of the data. If the DREM bit is cleared
            # logic 0 and one or more of the data ready event flags are enabled, then an
            # event flag will be raised whenever the system acquires a new set of data.
            ...
            self.bus.write_byte_data(self.ADDRESS, self.REG_PT_DATA_CONFIG, 0x00)
            self.is_setup = True
    ...
    def _read_data(self):
        ...
        try:
            ...
            if drStatus & 0x04 :  # new Pressure data is ready to be read
                OUT_P_MSB = self.bus.read_byte_data(self.ADDRESS, self.REG_PRESSURE_MSB)
                OUT_P_CSB = self.bus.read_byte_data(self.ADDRESS, self.REG_PRESSURE_CSB)
                OUT_P_LSB = self.bus.read_byte_data(self.ADDRESS, self.REG_PRESSURE_LSB)
            ...
            if drStatus & 0x02:  # new Temperature data is ready to be read
                OUT_T_MSB = self.bus.read_byte_data(self.ADDRESS, self.REG_TEMPERATURE_MSB)
                OUT_T_LSB = self.bus.read_byte_data(self.ADDRESS, self.REG_TEMPERATURE_LSB)

...
class TestAlarm(unittest.TestCase, Interruptable):
    ...
    def tearDownClass(self):
        self.logger.debug("tearDownClass started.")
    ...
    def setUp(self):
        self.logger.debug("Alarm_test: setUp")
        self.interrupts = 0
    ...
    def tearDown(self) -> 'None':
        self.logger.debug("Alarm_test: teardown")
        # watch to make sure that the alarm executes its callbacks.
    ...
    def test_a_json_pickle(self) -> 'None':
        ...
        self.logger.debug("----Start test_json_pickle----")
        ...
        self.assertEqual(
          self.alarm.blocked,
          unpickled_point.blocked,
        )

        self.assertEqual(
          self.alarm.acknowledged,
          unpickled_point.acknowledged,
        )

        self.assertEqual(
          self.alarm.enabled,
          unpickled_point.enabled,
        )

        self.assertEqual(
          self.alarm.consequences,
          unpickled_point.consequences,
        )

        self.assertEqual(
          self.alarm.more_info,
          unpickled_point.more_info,
        )

        self.assertEqual(
          self.alarm.state,
          unpickled_point.state,
        )

        self.assertEqual(
          self.alarm.on_delay,
          unpickled_point.on_delay
        )

        self.assertEqual(
          self.alarm.off_delay,
          unpickled_point.off_delay,
        )

        self.assertEqual(
          self.alarm._activation_time,
          unpickled_point._activation_time,
        )

        self.assertEqual(
          self.alarm._is_reset_time,
          unpickled_point._is_reset_time,
        )

        def test_a_yaml_pickle(self):
            # n.b. the a in the test name is so that this test gets executed
            # first, the Mock module doesn't seem to want to relent outside of
            # the 'with' block.
    ...
    def test_alarm_with_timer(self) -> 'None':
        self.logger.debug("---- Start test_alarm_with_timer ----")
          ...
          patch('pyAutomation.DataObjects.Alarm.time') as time_mock:
            ...
            datetime_mock.now = Mock(
              return_value=datetime.datetime.strptime(
                ...
                "%Y/%m/%d %H:%M:%S.%f"))
            self.logger.debug("0.0 second mark.")

            # Turn on alarm. it's got a 5 second delay so make sure the alarm
            # stays off.
            self.logger.debug("Turning on alarm.")
            ...
            datetime_mock.now = Mock(
              return_value=datetime.datetime.strptime(
                  ...
                  "%Y/%m/%d %H:%M:%S.%f"))
            self.logger.debug("1.0 second mark.")
            ...
            datetime_mock.now = Mock(
              return_value=datetime.datetime.strptime(
                  '2019/01/01 00:00:05.001',
                  "%Y/%m/%d %H:%M:%S.%f"))
            self.logger.debug("5.001 second mark.")
            ...
            datetime_mock.now = Mock(
              return_value=datetime.datetime.strptime(
                '2019/01/01 00:00:06.000',
                "%Y/%m/%d %H:%M:%S.%f"))

            self.logger.debug("6.00 second mark.")

            # Turn the alarm off and run the interrupt handler.
            self.logger.debug("Turning off alarm.")
            self.alarm.input = False

            # pretend to run the alarm handler..
            self.alarm.evaluate()

            # The alarm's got a 2 second delay so  make sure the alarm is still
            # on.
            self.assertTrue(self.alarm.active)
            ...
            self.logger.debug("7.0 second mark.")
            ...
            datetime_mock.now = Mock(
              return_value=datetime.datetime.strptime(
                '2019/01/01 00:00:08.001',
                "%Y/%m/%d %H:%M:%S.%f"))

            self.logger.debug("8.001 second mark.")
            ...
            self.logger.debug("9.0 second mark.")

            # acknowledge the alarm ito ensure that it's removed from the
            # active_alarm_list.
    ...
    def test_alarm_no_timer(self) -> 'None':
        self.logger.debug("---- Starting test_alarm_no_timer ----")
        self.interrupts = 0
